...
class ManualTfidfVectorizer:
    ...
    def transform(self, sentences: list[str]) -> sparse.csr_matrix:
        ...
        return sparse.csr_matrix((data, (rows, cols)), shape=(len(sentences), self.vocab_size))
...
class PlagiarismService:
    def __init__(self):
        ...
        self.vectorizer_path = os.path.join(self.data_dir, "tfidf_vectorizer_manual.pkl")
    ...
    def load_model(self):
        ...
        if os.path.exists(self.vectorizer_path) and os.path.exists(self.matrix_path):
            ...
            print(f"✅ Model Plagiarisme Manual dimuat! Ukuran matriks: {self.corpus_matrix.shape}")
        else:
            print("⚠️ Peringatan: Model Plagiarisme Manual belum dilatih.")
    ...
    def train_and_save_model(self, database_sentences: list[str]):
        ...
        print(f"✅ Training Manual Selesai! Disimpan ke {self.data_dir}")
    ...
    def check_batch_similarity(self, query_sentences: list[str], db_sentence_ids: list[int], threshold: float = 0.5) -> list:
        ...
        menggunakan model TF-IDF manual.
        """
        if self.vectorizer is None or self.corpus_matrix is None:
            raise Exception("Model belum dilatih.")

        # 1. Transformasi SEMUA kalimat sekaligus
        # Karena fungsi transform() manual sudah mendukung list of strings, 
        # ini akan langsung mengembalikan sparse matrix yang sudah ter-Normalisasi L2.
        query_vectors = self.vectorizer.transform(query_sentences)

        # 2. Kalikan matriks skripsi dengan matriks database (Dot Product)
        # Ingat: Vektor L2 ternormalisasi A dot B = Cosine Similarity
        # Hasilnya adalah matriks 2D berisi skor kemiripan untuk semua kalimat
        similarities = query_vectors.dot(self.corpus_matrix.T).toarray()

        # 3. Filter hasilnya berdasarkan threshold
        batch_results = []
        for i, sim_array in enumerate(similarities):
            ...
            # Urutkan masing-masing hasil kalimat dari persentase tertinggi
            suspicious_matches.sort(key=lambda x: x["similarity_pct"], reverse=True)
